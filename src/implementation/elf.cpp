...
namespace bbe::formats::elf::impl{
    void Elf::add_text(const Text& text){
        using namespace std::literals::string_view_literals;
        add_section(u8".text"sv,1/*binary data*/,text.text().data(),text.text().size(),0x1000,0x10,true,false,true,0,0);
        cppp::bytes& symtab = sym_tabs.emplace_back();
        for(const auto& [name,data] : text.exports()){
            symtab.appendl<std::uint32_t>(symbol_names.add(name));
