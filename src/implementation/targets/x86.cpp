...
namespace bbe::targets::x86::impl{
    ...
    namespace{
        class DataValue{
            ...
            std::uint32_t soff;
            public:
                DataValue(const TypeInfo& t) : _type(&t), soff(NSOFF){}
                DataValue() : _type(nullptr){}
        ...
        class FunctionCompiler{
            ...
            std::uint32_t sp = 0;
            std::unordered_map<const dfg::DataNode*,DataValue> values;
            std::vector<DataValue> arg_values;
            DataValue& new_value(const dfg::DataNode& nd,const TypeInfo& t){
                return values.try_emplace(&nd,t).first->second;
            }
            DataValue& new_value(const dfg::DataNode& nd){
                return new_value(nd,tdb[nd.return_type()]);
            }
            DataValue& no_value(const dfg::DataNode& nd){
                return values.try_emplace(&nd).first->second;
            }
            ...
            template<typename T>
            DataValue& encode_arith(const dfg::DataNode& node){
                DataValue& rv = new_value(node);
                const DataValue& lhs = compile_node(*node.parents()[0uz]);
                const DataValue& rhs = compile_node(*node.parents()[1uz]);
                into(lhs,x::reg::A);
                into(rhs,x::reg::C);
            ...
            template<typename T>
            DataValue& encode_comparison(const dfg::DataNode& node){
                DataValue& rv = new_value(node);
                const DataValue& lhs = compile_node(*node.parents()[0uz]);
                const DataValue& rhs = compile_node(*node.parents()[1uz]);
                into(lhs,x::reg::A);
            ...
            public:
                ...
                const DataValue& compile_node(const dfg::DataNode& dn){
                    if(auto it=values.find(&dn);it!=values.end()) return it->second;
                    switch(dn.operation()){
                        using enum dfg::NodeType;
                        case UINT32: case SINT32: {
                            DataValue& v = new_value(dn);
                        ...
                        case CALL_BUILTIN: {
                            switch(dn.primitive()){
                                ...
                                case 10:
                                case 11:
                                    return encode_arith<x::instructions::add>(dn);
                                case 20:
                                case 21:
                                    return encode_arith<x::instructions::sub>(dn);
                                case 30:{
                                    DataValue& ret = new_value(dn);
                                ...
                                case 50:
                                    return encode_comparison<x::instructions::set::e>(dn);
                                case 51:
                                    return encode_comparison<x::instructions::set::le>(dn);
                                default:
    ...
    Function::Function(cppp::sv cn,const dfg::Function& f,const TypeDatabase& tdb) : _cname(cn){
        ...
        const DataValue& retv{compiler.compile_node(f.dfg().root().value())};
        if(retv.is_pack()){
